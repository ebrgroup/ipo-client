...
const Copyright_Classification = ({ Progress }) => {
    ...
    const [theClassification, setTheClassification] = useState(null); 
    ...
    const handleDataAndNavigation = () => {
        if (classificationDescription !== "" && theClassification && theClassification.id) {
            const classificationData = {
                classificationClass: theClassification.id,
    ...
    useEffect(() => {
        ...
        if (data) {
            ...
            for(let obj of classifications) {
                if(obj.id === classificationClass) {
                    setTheClassification(obj);
                    break;
                }
            }
    ...
    return (
        <div className="classificationBox">
            ...
            <div style={{ height: "74%", gap: "1.25rem", display: "flex", flexDirection: "column", overflow: "auto" }}>
                ...
                {theClassification && <div style={{ fontSize: "0.75rem", textAlign: "justify", width: "87%" }}>
                    <span className="classificationLabel">
                        Selected Class
                    </span>
                    <br />
                    <div className="classDesc">
                        <span>
                            <b>{theClassification.id} -</b> {`${theClassification.description}`}
                        </span>
                    </div>
                </div>}
                <div>
